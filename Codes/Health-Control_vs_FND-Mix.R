...
##### Loading Results ##########################################################
load(paste0(Input_Path,"MCMC_Health-Control_vs_FND-Mixed.Rdata"))


Graph_Creation_Fud2<-function(Post,Dict){
...
Graph1=Graph_Creation_Fud2(Post1,Dict)
Graph2=Graph_Creation_Fud2(Post2,Dict)

V(Graph1)$Type="FND"
V(Graph2)$Type="Health"
...
perm=as.vector(sapply(c("aDMN","SMA","pDMN"),function(v) which(c(V(Graph1)$Region)==v)))
...
nodes$color <- factor(nodes$Region,labels = rainbow(length(unique(nodes$Region))))
edges$color <- ifelse(edges$weight == 1, "red", "blue")
nodes$label<-nodes$Metabolite
edges$width_ori=E(Graph1)$weight
...
write_xlsx(Connection_Table,file=paste0(Output_Path,"ConTable_FND.xlsx"))
...
perm=as.vector(sapply(c("aDMN","SMA","pDMN"),function(v) which(c(V(Graph2)$Region)==v)))
...
edges$color <- ifelse(edges$weight == 1, "red", "blue")
nodes$label<-nodes$Metabolite
edges$width_ori=E(Graph2)$weight
...
write_xlsx(Connection_Table,file=paste0(Output_Path,"ConTable_Healthy.xlsx"))
